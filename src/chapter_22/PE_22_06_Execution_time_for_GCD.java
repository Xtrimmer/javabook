...
public class PE_22_06_Execution_time_for_GCD {
    ...
    private static void printGCDEuclidTimes(int[] fibNumbers) {
        for (int i = 1; i < fibNumbers.length; i++) {
            long startTime = System.currentTimeMillis();
            GCDEuclid.gcd(fibNumbers[i - 1], fibNumbers[i]);
            long endTime = System.currentTimeMillis();
            long executionTime = endTime - startTime;
    ...
    private static void printGCDTimes(int[] fibNumbers) {
        for (int i = 1; i < fibNumbers.length; i++) {
            long startTime = System.currentTimeMillis();
            GCD.gcd(fibNumbers[i - 1], fibNumbers[i]);
            long endTime = System.currentTimeMillis();
            long executionTime = endTime - startTime;
            System.out.printf("%-8d", executionTime);
        }
    ...
    private static void printGcdTable(int[] fibNumbers) {
        ...
        printGCDTimes(fibNumbers);
        System.out.print("Listing 22.4 GCDEuclid | ");
        printGCDEuclidTimes(fibNumbers);
    }
}
